        ...
        <motion.div variants={tile} className="pm-tile p-3 group">
            ...
            <div className="flex items-center justify-between mb-2.5">
                ...
                <div className="flex items-center gap-1.5">
                    ...
                    {isLive && (
                        <span className="pm-live-badge pm-badge">
                    ...
                    {isScheduled && confidence === "high" && (
                        <Tooltip content="High model confidence">
                            <span className="pm-badge bg-accent/10 text-accent border border-accent/20 flex items-center gap-1">
                                <Flame size={8} />
                                Edge
                            </span>
                        </Tooltip>
                    ...
                    {isScheduled && confidence !== "high" && (
                        <span className="pm-badge bg-pitch-700 text-pitch-400 border border-pitch-600">Tonight</span>
                    )}
                </div>
            </div>
            ...
            <div className="flex items-center gap-2 mb-3">
                ...
                <div className="flex-1 text-center">
                    <div
                        className={`font-display text-xl tracking-wider leading-none mb-0.5 transition-colors
              ${fav === "away" ? "" : "text-pitch-300"}`}
                        style={{ color: fav === "away" ? awayColor : undefined }}
                    >
                        {game.away}
                    </div>
                    <div className="text-[10px] text-pitch-500 truncate px-1">
                    ...
                    {(isFinal || isLive) ? (
                        <motion.div
                            ...
                            initial={{ scale: 1.1 }}
                            animate={{ scale: 1 }}
                            className="pm-number text-lg mt-1 text-pitch-100"
                        >
                            {game.awayScore}
                        </motion.div>
                    ) : (
                ...
                <div className="flex flex-col items-center gap-0.5">
                    <div className="text-pitch-600 text-xs font-mono">
                        {isFinal || isLive ? "—" : "vs"}
                    </div>
                    {isScheduled && game.spread !== "—" && (
                        <div className="text-[9px] text-pitch-600">{game.spread}</div>
                    )}
                </div>
            ...
            {isScheduled && (
                <div className="mb-2.5">
                    <div className="flex justify-between text-[9px] text-pitch-600 mb-1">
                        <span>{game.away}</span>
                        <span>{game.home}</span>
                    </div>
                    <div className="h-1.5 rounded-full bg-pitch-700 overflow-hidden relative">
                        <motion.div
                            ...
                            style={{ background: awayColor, opacity: 0.85 }}
                            initial={{ width: 0 }}
                            animate={{ width: `${game.awayP}%` }}
        ...
        <div>
            <div className="flex items-center justify-between mb-2">
                ...
                <button
                    ...
                    className="text-[9px] text-pitch-500 hover:text-accent transition-colors flex items-center gap-0.5"
                >
            ...
            <div className="space-y-0">
                ...
                transition-colors hover:bg-pitch-700 cursor-pointer group
                ${i === 6 ? "border-t border-dashed border-pitch-600 mt-1 pt-2.5" : ""}`}
                        >
                            ...
                            <div
                                ...
                                style={{ background: color || "#546480" }}
                            />

                            <span className={`font-display text-sm tracking-wider flex-1
                ${i === 0 ? "text-accent" : isPlayoff ? "text-pitch-200" : "text-pitch-400"}`}>
                                {t.team}
                            </span>
                  ...
                  ${t.streak.startsWith("W") ? "text-win/60" : "text-loss/60"}`}>
                                    {t.streak}
                                </span>
                            )}
                            ...
                            {isPlayIn && (
                                <span className="text-[8px] text-draw border border-draw/30 bg-draw/10 px-1 py-0.5 rounded flex-shrink-0">
                                    PI
                                </span>
                            )}
                        </div>
...
function SummaryTile({ label, value, sub, icon: Icon, trend, color, onClick, badge }) {
    return (
        <motion.div
            variants={tile}
            className={`pm-tile p-4 ${onClick ? "cursor-pointer" : ""}`}
            onClick={onClick}
        ...
        >
            <div className="flex items-start justify-between mb-2.5">
                ...
                {Icon && (
                    <div className="w-7 h-7 rounded-md bg-pitch-750 border border-pitch-600 flex items-center justify-center flex-shrink-0">
                        <Icon size={13} strokeWidth={1.8} className="text-pitch-400" />
                    </div>
                )}
            </div>
            ...
            {(sub || trend || badge) && (
                <div className="flex items-center gap-1.5 mt-2">
                    ...
                    {badge && (
                        <span className={`pm-badge text-[9px] px-1.5 py-0.5 ${badge.cls}`}>{badge.label}</span>
                    )}
...
function KellyTile({ topEdge, bankroll }) {
    const kelly = kellyBet(topEdge.modelP / 100, -110, bankroll || DEFAULT_BANKROLL);
    ...
    return (
        <motion.div variants={tile} className="pm-tile p-4">
            ...
            <div className="pm-number text-3xl text-pitch-50">
                {kelly > 0 ? formatCurrency(kelly, false) : "—"}
            </div>

            <div className="flex items-center gap-1.5 mt-2">
                <Shield size={10} className="text-pitch-500 flex-shrink-0" />
                <span className="text-[11px] text-pitch-500">
                    ½-Kelly · {topEdge.matchup}
                </span>
            </div>
        </motion.div>
    );
...
export default function Dashboard({ onNavigate }) {
    ...
    const gameList = useMemo(
        () => mergeOddsIntoGames(rawGameList, oddsData) || [],
        [rawGameList, oddsData]
    );
    ...
    // Bet stats — re-reads on storage events
    ...
    const topEdge = useMemo(() => {
        if (oddsData && Object.keys(oddsData).length > 0) {
            ...
            return cards.reduce((best, g) => (g.modelP - g.impliedP) > (best.modelP - best.impliedP) ? g : best,
                { matchup: "—", fav: "—", modelP: 0, impliedP: 0 });
        }
    ...
    return (
        ...
        >
            ...
            <motion.div variants={tile} className="col-span-12">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    <SummaryTile
                        label="Games Today"
                        value={gameCount || "—"}
                        sub={liveCount > 0 ? `${liveCount} in progress` : "Tip-off tonight"}
                        icon={Target}
                        trend={liveCount > 0 ? "up" : null}
                        badge={liveCount > 0 ? { label: "LIVE", cls: "bg-win/10 text-win border border-win/20" } : null}
                        onClick={() => onNavigate("scores")}
                    />
                    <SummaryTile
                        label="Top Model Edge"
                        value={edgeDiff > 0 ? `+${edgeDiff.toFixed(1)}%` : "—"}
                        sub={topEdge.matchup}
                        icon={TrendingUp}
                        trend="up"
                        color={edgeDiff >= 10 ? "text-win" : edgeDiff >= 5 ? "text-draw" : "text-pitch-50"}
                        onClick={() => onNavigate("betting")}
                    />
                    <SummaryTile
                        label="Best Win Prob"
                        value={bestProb.modelP > 0 ? `${bestProb.modelP}%` : "—"}
                        sub={bestProb.fav !== "—" ? `${bestProb.fav} · ${bestProb.matchup}` : "No data"}
                        icon={Zap}
                        onClick={() => onNavigate("betting")}
                    />
                    <SummaryTile
                        label="Net P&L"
                        value={betStats.total > 0 ? formatCurrency(betStats.pl) : "—"}
                        sub={betSub}
                        icon={BarChart2}
                        color={betStats.pl > 0 ? "text-win" : betStats.pl < 0 ? "text-loss" : "text-pitch-50"}
                        trend={betStats.pl > 0 ? "up" : betStats.pl < 0 ? "down" : null}
            ...
            <div className="col-span-12 lg:col-span-8">
                <div className="flex items-center justify-between mb-2">
                    <div className="pm-label">
                        Today's Games
                        {liveCount > 0 && (
                            <span className="ml-2 pm-badge bg-win/10 text-win border border-win/20 inline-flex items-center gap-1">
                                <span className="pm-live-dot" />
                                {liveCount} live
                            </span>
                        )}
                    ...
                    <button
                        onClick={() => onNavigate("scores")}
                        className="flex items-center gap-1 text-[10px] text-pitch-500 hover:text-accent transition-colors"
                    >
                ...
                {!HAS_API_KEY ? (
                    ...
                    <div className="pm-card p-12 text-center">
                        <Activity size={28} className="text-pitch-700 mx-auto mb-3" />
                        <div className="text-sm text-pitch-500">No games scheduled today.</div>
                        <div className="text-[11px] text-pitch-600 mt-1">Check back later for upcoming matchups.</div>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                        <AnimatePresence>
                ...
                {gameList.length > 6 && (
                    <button
                        onClick={() => onNavigate("scores")}
                        className="mt-3 w-full pm-btn-ghost text-center justify-center"
                    >
                        +{gameList.length - 6} more games <ChevronRight size={13} />
                    </button>
                )}
            </div>
            ...
            <div className="col-span-12 lg:col-span-4 space-y-3">
                ...
                {/* Kelly Criterion tile (only shows with real odds) */}
                {oddsData && Object.keys(oddsData).length > 0 && topEdge.modelP > 0 && (
                    <KellyTile topEdge={topEdge} bankroll={DEFAULT_BANKROLL} />
                )}
                ...
                <motion.div variants={tile} className="pm-card p-4">
                    <div className="flex items-center justify-between mb-3">
                        ...
                        <button
                            onClick={() => onNavigate("standings")}
                            className="text-[10px] text-pitch-500 hover:text-accent transition-colors flex items-center gap-0.5"
                        >
                            Full table <ChevronRight size={10} />
                        </button>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        {[
                            ...
                            <button
                                ...
                                className="flex items-center gap-2 px-3 py-2.5 rounded-lg bg-pitch-750 border border-pitch-700
                           hover:border-pitch-500 hover:bg-pitch-700 transition-all group text-left"
                            >
                                <item.icon size={13} strokeWidth={1.8} className={`${item.color} flex-shrink-0`} />
                                <span className="text-[11px] text-pitch-300 group-hover:text-pitch-100 transition-colors font-medium">
                                    {item.label}
                                </span>
                            </button>
                        ))}
