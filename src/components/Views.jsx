...
import { TODAY_GAMES, TEAM_NAMES, EAST_STANDINGS, WEST_STANDINGS, ODDS_GAMES } from "../data";
...
export function Scores() {
    ...
    return (
        <motion.div variants={container} initial="hidden" animate="show">
            <div className="pm-label mb-3">Tonight · Mar 19, 2026 · {TODAY_GAMES.length} games</div>
                    ...
                    <thead>
                        <tr className="border-b border-pitch-600">
                            {["#", "Team", "W", "L", "PCT", "L10", "HOME", "ROAD", "STREAK"].map(h => (
                                <th key={h} className="px-3 py-2.5 text-left pm-label font-medium">
                                    {h}
                                </th>
                            ))}
                        </tr>
                        ...
                        <motion.div key={g.matchup} variants={item} className="pm-tile p-4">
                            <div className="flex items-start justify-between mb-3">
                                <div>
                                    ...
                                    <div className="text-[10px] text-pitch-400 mt-0.5">Favourite: <span className="text-accent">{g.fav}</span></div>
                                </div>
                            ...
                            <div className="space-y-1.5 mb-3">
                                ...
                                <div className="flex justify-between text-[11px]">
                                    ...
                                    <span className={`font-mono font-medium ${diff >= 10 ? "text-win" : diff >= 5 ? "text-draw" : "text-pitch-400"}`}>
                                        +{diff}%
                                    </span>
                                </div>
                            </div>
...
function calcPL(stake, odds, result) {
    ...
    return o > 0
        ? +(parseFloat(stake) * o / 100).toFixed(2)
        : +(parseFloat(stake) * 100 / Math.abs(o)).toFixed(2);
}
...
export function BetTracker() {
    ...
    const roi = staked > 0 ? (totalPL / staked * 100) : 0;

    // ROI chart data
    let running = 0;
    const chartData = [...bets].filter(b => b.result !== "pending").reverse().map((b, i) => {
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <div className="pm-card p-4 mb-4">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mb-3">
                    ...
                    <input className={inputCls} placeholder="Pick (e.g. BOS -9.5)" value={form.pick} onChange={e => setForm(f => ({ ...f, pick: e.target.value }))} />
